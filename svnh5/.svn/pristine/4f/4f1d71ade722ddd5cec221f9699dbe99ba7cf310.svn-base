...
{  
    ...
    { 
        ...
        //资源重新加载
        SssResMgr.getInstance().clear();  
        var myseatid=RoomMgr.getInstance().getMySeatId(); 
        this.myself.setEvents(msg.events);
        this.myself.replaceJin();
        SssResMgr.getInstance().setJin(this.jin)
    }
    tuoGuan( bvalue)
    {
        // body
    ...
    resetData(  )
    {
        ...
        this.process=SssDef.process_ready;
    }
    //玩家操作
    onOp(msg)
    {
        ...
        else if( op==SssDef.op_hu ){ 
            this.op_hu(msg);
        }
        else if(op==SssDef.op_qianggang_hu){
            this.op_qianggang_hu(msg);
        }
        else if( op==SssDef.op_peng ){ 
            this.op_peng(msg);
        }
        else if( op==SssDef.op_gang ){ 
            this.op_gang(msg);
        }
        else if( op==SssDef.op_angang ){ 
            this.op_angang(msg);
        }
        else if( op==SssDef.op_chi ){ 
            this.op_chi(msg);
        }
        else if( op==SssDef.op_zimo ){ 
            this.op_zimo(msg); 
        }
        else if( op==SssDef.op_bugang){ 
            this.op_bugang(msg); 
        }
    }
    ...
    {
        this.process=msg.process; 
        if (this.process==SssDef.process_dingzhuang){ 
            this.process_dingzhuang(msg);
        }
        else if( this.process==SssDef.process_fapai){ 
            this.process_fapai(msg);
        }
        else if( this.process==SssDef.process_buhua){ 
            this.process_buhua(msg);
        }
        else if( this.process==SssDef.process_kaijin){ 
            this.process_kaijin(msg); 
        }
        else if( this.process==SssDef.process_ready){ 
