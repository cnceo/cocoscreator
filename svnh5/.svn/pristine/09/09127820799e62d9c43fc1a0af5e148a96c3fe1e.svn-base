...
class View extends BaseView{
	...
	showComponents(){
		...
		{ 
			let rppnode=rpparr[i];
			let prefab=prefabarr[i]; 
	...
	{ 
		...
		this.ui.img_baoxianghao=ctrl.img_baoxianghao;
		...
		if(this.model.roomtype!=2)
		{
			this.ui.panel_round.active=false;
			this.ui.lbl_roomid.node.active=false; 
			this.ui.img_baoxianghao.active=false;  
		} 
		if(this.model.roomtype==2){ 
	...
	updateRoomId(){
		this.ui.img_baoxianghao.active=true;
		var roominfo=RoomMgr.getInstance().roominfo;
		this.ui.lbl_roomid.node.active=true 
		console.log("updateRoomId {roominfo=",roominfo)
		this.ui.lbl_roomid.string= `${roominfo.password}`;
	}
	updateLeftCardCount(  )
	{ 
...
export default class QzmjRoomCtrl extends BaseCtrl {
	...
	private btn_setting_cb () : void {
		 this.start_sub_module(G_MODULE.QzmjRoomSetting);
	...
	btn_exit_cb( ){ 
		...
		if (RoomMgr.getInstance().bGameIsStated) {
			...
			if (this.model.owner==UserMgr.getInstance().getUid()){
				var okcb=function(  )
				{
					// body
