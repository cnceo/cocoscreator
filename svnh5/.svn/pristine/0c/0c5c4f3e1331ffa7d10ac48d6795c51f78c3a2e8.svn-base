...
class Model extends BaseModel{
    public musicVolume = QgmjAudio.getInstance().musicVolume;
    public effectVolume = QgmjAudio.getInstance().effectVolume;
    public musicPathKing = QgmjAudio.getInstance().musicPathKing;
	constructor()
	{
		super();
	}
...
export default class Prefab_RoomSettingCtrl extends BaseCtrl {
	...
	private MandarinLanguage_cb () : void {
		...
		QgmjAudio.getInstance().setMusicPathKind(1)
	}
	private LocalLanguage_cb () : void {
		console.log("LocalLanguage_cb")
		QgmjAudio.getInstance().setMusicPathKind(0)
	}
	...
	private _onMusic (event) : void {
		...
		QgmjAudio.getInstance().setMusicVolume(slider.progress);
	}
	private _onEffect (event) : void {
		var slider = event.detail;
		this.EffectProgress.progress = slider.progress;
		QgmjAudio.getInstance().setEffectVolume(slider.progress);
	}
