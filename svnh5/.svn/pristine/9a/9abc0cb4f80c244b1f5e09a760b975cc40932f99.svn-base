...
export default class Prefab_ChatCtrl extends BaseCtrl {
	...
	defineGlobalEvents()
	{
		this.n_events = {
			...
			'http.reqChat':this.http_reqChat.bind(this),
	...
	http_reqChat(msg){
		...
		let logicSeatId = RoomMgr.getInstance().users;
		debugger;
		RoomMgr.getInstance().getViewSeatId(logicSeatId);
		let type = 1
		switch(type){
			case 0:
			this.rpp_chat0.removeAllChildren();
			this.rpp_chat0.addChild(prefab_ChatMsg);
			break;
			case 1:
			this.rpp_chat1.removeAllChildren();
			this.rpp_chat1.addChild(prefab_ChatMsg);
			prefab_ChatMsg.getChildByName("chat_background").getChildByName("background").scaleX = -1;
			break;
			case 2:
			this.rpp_chat2.removeAllChildren();
			this.rpp_chat2.addChild(prefab_ChatMsg);
			prefab_ChatMsg.getChildByName("chat_background").getChildByName("background").scaleX = -1;
			break;
			case 3:
			this.rpp_chat3.removeAllChildren();
			this.rpp_chat3.addChild(prefab_ChatMsg);
			break;
		}
		//uid":"101","rid":"10086","cid":"5"
		if(parseInt(msg.cid) >=1000){
			//表情
			let i = parseInt(msg.cid)-1000
			prefab_ChatMsg.getComponent(prefab_ChatMsg.name).setImg(i)
		}else{
			//文本
			let i = parseInt(msg.cid);
			prefab_ChatMsg.getComponent(prefab_ChatMsg.name).setMsg(i);
			QzmjAudio.getInstance().playVoiceAudio(i);
		}
	}
	//end
	//全局事件回调begin
