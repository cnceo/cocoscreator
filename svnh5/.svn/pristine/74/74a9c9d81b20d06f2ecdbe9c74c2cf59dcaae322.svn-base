...
import Prefab_bull_calculateCtrl from "../../Tbnn/BullModules/Prefab_bull_calculateCtrl";
...
class View extends BaseView{
	...
	initUi()
	{
        ...
        this.model.seatCount = this.ui.node_givePos.children.length;
    }
    ...
    showCard(cardsNum:number){
        ...
        for(i = 0; i < this.model.maxHandlerCardNum; i ++){
            ...
            if(cardsNum > 0 && cardNode.active == false){
                ...
                CardsMgr.setCardValue(cardNode);
            }
        } 
    }
    ...
    openAllCards (valueList){
        CardsMgr.openCards(this.list_myCards, valueList);
    }
    ...
    //盖住所有的卡牌
    coverAllCards(){
        CardsMgr.coverCards(this.list_myCards);
    }
    ...
    showTiShi (){
        ...
        let resultObj = CardsMgr.getCardResult(cardsList);
...
export default class QznnMyCardsCtrl extends BaseCtrl {
	...
	onLoad (){
        ...
        CardsMgr.initData(this, this.ui.atlas_cards);
	}
    ...
    */
    onTanPai(msg){
        ...
        if(msg.seatId == RoomMgr.getInstance().myseatid){
            ...
            CardsMgr.showTanPai(this.view.list_myCards, msg.cardLogicIdList, targetPos);
        }
    }
    /*结算
    ...
    */
    onSettle_bull(msg){
        ...
        if(resultObj){
            let targetPos = this.view.getGiveTargetPos(this.model.myViewSeatId);
            CardsMgr.showTanPai(this.view.list_myCards, resultObj.cardIdList, targetPos);
    ...
    public onTouchEnd(event){
        ...
        for(let i = 0; i < cards.length; i ++){
            cards[i].opacity = 255;
            console.log('click card VALUE= ', CardsMgr.getSixValue(cards[i][C_cardAttr].logicValue));
    ...
    private _getCardValue(logicValue:number){
        let sixValue:string = CardsMgr.getSixValue(logicValue);
