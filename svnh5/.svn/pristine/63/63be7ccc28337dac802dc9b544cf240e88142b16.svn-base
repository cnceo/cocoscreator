...
class View extends BaseView{
    ...
    showCard(viewSeatId:number, cardsNum:number){
        ...
        for(i = 0; i < this.model.maxHandlerCardNum; i ++){
            ...
            if(!cardNode){
                cardNode = CardsMgr.addOtherCard();
    ...
    coverAllCards(viewSeatId:number){
        CardsMgr.coverCards(this.dict_cards[viewSeatId]);
    }
    ...
    */
    onTanPai(msg){
        ...
        if(id != RoomMgr.getInstance().myseatid){
            ...
            CardsMgr.showTanPai(nodeList, msg.cardLogicIdList, targetPos);
        }
    }
    /*结算
    ...
    onSettle_bull(msg){
        ...
        for(seatId in msg.dict_notTanPai){
            if(seatId != myLogicId){
                ...
                let targetPos = this.view.getGiveTargetPos(viewSeatId);
                CardsMgr.showTanPai(this.view.getCardListByViewId(viewSeatId), resultObj.cardIdList, targetPos);
            }
        }
    }
	//end
