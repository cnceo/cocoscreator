...
let ctrl : Prefab_QgmjGmPlayerCtrl;
//模型，数据处理
class Model extends BaseModel{
	...
	getMyCards(){
		...
		let players=QgmjLogic.getInstance().players;
...
class View extends BaseView{
	...
	createMahjong (id, parent) : void {
		...
		let sprite = new cc.SpriteFrame(QgmjResMgr.getInstance().getCardTextureByValue(id));
...
//c, 控制
@ccclass
export default class Prefab_QgmjGmPlayerCtrl extends BaseCtrl {
	//这边去声明ui组件
	@property({
		tooltip : "我方的麻将位置",
		type : cc.Node
	...
	start () {
		let msg = {
			reqtype : QgmjDef.gmreq_cards, 
			data : {
				target : BehaviorMgr.getInstance().changeSeatId
			}
		};
		QgmjLogic.getInstance().gmReq(msg);
	}
	//网络事件回调begin
	...
	emitChangeCard () : void {
		if (this.model.handCard != null && this.model.oppositeCard != null) {
			let msg={ 
				optype : QgmjDef.gmop_changecard, 
				data:{
			...
			QgmjLogic.getInstance().gmOp(msg);
			
			// let node = new cc.Node();//测试用的
			// cc.find("Canvas").addChild(node);
			// let s = new cc.SpriteFrame(QgmjResMgr.getInstance().getCardTextureByValue(this.model.handCard));
