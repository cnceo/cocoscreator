...
export default class Prefab_op_animCtrl extends BaseCtrl {
	...
	onOp(msg){
		...
		//判断出牌
		if(QgmjDef.op_cfg[msg.event] == QgmjDef.op_chupai) {
		...
		console.log(`${this.node.name}==op_anim${seatId}`)
		let op=QgmjDef.op_cfg[msg.event]
		let index = 1;
		switch(op){
			case QgmjDef.op_chi:
			index = 0;
			...
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_zimo:
			index = 4;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_sanjindao:
			index = 5;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_qianggang_hu:
			index = 6;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_danyou:
			index = 7;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_shuangyou:
			index = 8;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_sanyou:
			index = 9;
			this.view.runAnimByIndex(index);
			break;
			case QgmjDef.op_bazhanghua:
			index = 10;
			this.view.runAnimByIndex(index);
			break;
			default:
