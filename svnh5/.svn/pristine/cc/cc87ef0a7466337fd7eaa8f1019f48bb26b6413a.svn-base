...
class Model extends BaseModel{
	...
	updateZhuang()
	{ 
		this.isZhuangJia=SssLogic.getInstance().zhuangseat==this.logicseatid;
	}
	recover()
	{
		this.updateZhuang();
...
export default class SssSeatCtrl extends BaseCtrl {
	...
	onProcess(msg)
	{ 
		if (msg.process==SssDef.process_dingzhuang)
		{
	...
	onLeaveRoom(msg){ 
		if (this.model.logicseatid==msg.seatid){
			this.model.clear(); 
			this.view.clear();
			SssAudio.getInstance().OtherPlayerLeaveAudio();
			console.log("leave")
		}
	}
	onEnterRoom(msg){ 
		...
		SssAudio.getInstance().OtherPlayerEnterAudio();
		console.log("Enter")
	}
