...
class View extends BaseView{
    ...
    initCheckState(groupChildren, toggleName) {
    	...
    	for (let i=0; i<data.length; i++) {
            ...
            if (toggleName == 'v_roundcount'){
                groupChildren[i].getChildByName('lab3').getComponent(cc.Label).string = "×"+ this.model.roomRuleInfo.v_fangfei;
            }
    	}
    }
    ...
    refreshFangfei(roomcost){
        ...
        for(let i = 0; i<groups.length; i++){
            if (groups[i].name == 'v_roundcount'){
                let groupChildren = groups[i].getChildByName('ToggleContainer').children;
                console.log()
                for (let j = 0; j < groupChildren.length; j++) {
                    groupChildren[j].getChildByName('lab3').getComponent(cc.Label).string = "×"+ roomcost;
                }
            }
        }
    }
...
export default class Prefab_CreateQzmj extends BaseCtrl {
    ...
    connectUi () {
    	...
    	for(let i = 0; i<groups.length; i++){
    		...
    		for(let j = 0; j<groupChildren.length; j++){
    			...
    			else if (groups[i].name == "v_paytype") callback = this.payTypeCB
    ...
    refFangfei(){
        ...
        CreateRoomMgr.getInstance().setProperty(roomCost, 'qzmjRoomRuleInfo', 'v_fangfei')
    ...
    gameCountCB(event) {
        ...
        let yikeValue = (value==0)?1:0;
        console.log('是否为一课', yikeValue)
        CreateRoomMgr.getInstance().setProperty(yikeValue, 'qzmjRoomRuleInfo', 'b_yike')
        this.refFangfei()//刷新房费数据及界面显示数值
    ...
    payTypeCB(event){
    	let index = Number(event.currentTarget.name-1)
    	let value = this.model.roomcfg['v_paytype'][index]
    	this.model.roomRuleInfo.v_paytype = value;
    	CreateRoomMgr.getInstance().setProperty(value, 'qzmjRoomRuleInfo', 'v_paytype')
        this.refFangfei()
    	console.log(this.model.roomRuleInfo.v_paytype)
    },
    youjinTypeCB(event){
    	let index = Number(event.currentTarget.name-1)
