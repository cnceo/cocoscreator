...
export default class Prefab_MusicSettingCtrl extends BaseControl {
	...
	connectUi()
	{
		...
		this.connect(G_UiType.toggle, this.ui.button_TopolectSwitch, this.SwitchCB, '方言设置');
	}
	start () {
	}
	//网络事件回调begin
	...
	private SwitchCB(event){
		...
		switch (event.currentTarget.name) {
			case 'musicSwitch':
				...
				QzmjAudio.getInstance().setMusicVolume(prog);
				break;
			case 'effectSwitch':
				let bEffectSwitch = this.model.musicSettingInfo.bEffectSwitch
				...
				QzmjAudio.getInstance().setEffectVolume(prog);
				break;
			case 'yySwitch':
				let bYySwitch = this.model.musicSettingInfo.bYySwitch
			...
			case 'topolectSwitch':
				...
				QzmjAudio.getInstance().setMusicPathKind(this.model.musicSettingInfo.bTopolectSwitch);
				break;
		}
	...
	private VolumeCB(event){
		...
		if (event.currentTarget.name == 'musicSlider'){
			...
			QzmjAudio.getInstance().setMusicVolume(this.ui.node_MusicVolume_Progress.progress);
		}
			...
			QzmjAudio.getInstance().setEffectVolume(this.ui.node_EffectVolume_Progress.progress);
		}
			...
			QzmjAudio.getInstance().setEffectVolume(this.ui.node_yyVolume_Progress.progress);
		}
		this.model.musicSettingInfo = SettingMgr.getInstance().getMusicInfo();
		console.log('musicInfo:',this.model.musicSettingInfo);
	}
