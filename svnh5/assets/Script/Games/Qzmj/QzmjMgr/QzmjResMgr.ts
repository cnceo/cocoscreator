...
export default class QzmjResMgr{
    ...
    {
        let index = QzmjResMgr.huaIcons[value];  
        var texture=cc.loader.getRes(cc.url.raw(`resources/Games/Qzmj/BuhuaStatus_${index}.png`));     
        return texture;
    }
    ...
    public getAudioByName(name:string){
        var audioList= QzmjResMgr.OpAudioPath[name];
        if(!audioList){
    ...
    public getProcessAudioByName(name:string){
        var audioList= QzmjResMgr.ProcessAudioPath[name];
        if(!audioList){
            console.log(`#error name = ${name}`);
            return;
        }
    ...
    public getCardAudioByName(opName:string,cardName:string){
        var cardAudioList= QzmjResMgr.OpAudioPath[opName];
        if(!cardAudioList){
