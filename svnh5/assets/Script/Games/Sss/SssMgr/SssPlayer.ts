...
export default class SssPlayer
{
	deposit=false;//是否托管  
	tingarr=[]; 
	handcard=[];
	...
	logic=null;
	events=[];//事件列表
	state=SssDef.state_idle;//玩家初始状态
	constructor()  
	{ 
		this.uid=null;//uid  
		this.seatid=null 
	...
	{
		this.events=[];
		this.state=SssDef.state_idle;//玩家初始状态
	}
	pushEvent(event)
	{
		//推入事件
		this.events.push(event);
		this.state=SssDef.state_idle;
		switch(event)
		{
			//如果是出牌事件，则将状态切至出牌
			case SssDef.event_chupai:
				this.state=SssDef.state_chupai;
			break;
			//如果是其他事件则将状态切入事件
			default:
				this.state=SssDef.state_event;
			break;
		} 
	}
	setEvents(events)
	...
	ResolvePai(  pai,   joker_count,hucards)
	{
	    ...
	    if (this.Majiang_Type(pai[0]) < SssDef.MajiangType.emMJType_Zi)
	    {
	...
	IsCommonHu( original_pai)
	{
	    ...
	    while (true)
	    {
	        ...
	        for (; i < pai.size(); ++ i)
	        {
	            ...
	            {
	                //加入记录
	                if(pai[i]==SssDef.emMJ.emMJ_Joker)
	                {
	                    hucards.push({
	                        type:0,
	                        jincount:2,
	            ...
	            else if (pai[i] != SssDef.emMJ.emMJ_Joker && pai[0] == SssDef.emMJ.emMJ_Joker)
	            {
	                hucards.push({
	                    type:0,
	                    jincount:1,
	        ...
	        while (joker_end != pai.end() && pai[joker_end] == SssDef.emMJ.emMJ_Joker)
	        {
	            ++joker_end;
	        }
	        ...
	        else
	        {
	            ...
	            while (pai.size() >= 3)
	            {
	                ...
	                else if (this.Majiang_Type(pai[0]) < SssDef.MajiangType.emMJType_Zi)
	                {
	                    var it1 = this.Find_In_Sorted(pai,pai.begin() + 1, pai.end(), pai[0] + 1);
	...
	getTingArr(tmpPai)
	{
	    ...
	    while (joker_end != pai.cend() && pai[joker_end] == SssDef.emMJ.emMJ_Joker)
	    {
	        ++joker_end;
	    }
		//算金个数
	    ...
	    for (var k=0;k<SssDef.ting_majiang_types.length;++k)
	    {
	        var i=SssDef.ting_majiang_types[k]
	        // 金转换
	        if (i==this.logic.jin) {
	            continue;
	        }
	        ...
	        if(jocker_count  == 0)
	        {
	            ...
	            if (this.Majiang_Type(i) >= SssDef.MajiangType.emMJType_Zi)
	            {
	                //字牌必须有相同的才可能和
	...
	{ 
		for (let i = 0; i < this.opcards.length; i++) {
			let opinfo=this.opcards[i]
			if(opinfo.op==SssDef.op_bugang&&opinfo.value==card)
			{
				opinfo.op=SssDef.op_peng;
			}
		}
	} 
	...
	{ 
		for (let i = 0; i < this.opcards.length; i++) {
			let opinfo=this.opcards[i]
			if(opinfo.op==SssDef.op_peng&&opinfo.value==card)
			{
				opinfo.op=SssDef.op_bugang;
			}
		}
	} 
	//加入杠
	...
	getCardsCandChi()
	{
		...
		for(let index=0;index <SssDef.chiarr.length;++index)
		{
			var arr=SssDef.chiarr[index];
	...
	{	
		...
		for (var cardvalue in cardcountmap){
			...
			if (count==4)
			{
				console.log("SssPlayer:getCardsCanAnGang",cardvalue)
	...
	getCardsCanBuGang(){
        ...
        for (let index = 0; index < this.handcard.length; index++) {
            ...
            for (let i = 0; i < this.opcards.length; i++) {
                let opinfo=this.opcards[i] 
                if(opinfo.op==SssDef.op_peng&&opinfo.value==cardValue)
                {
