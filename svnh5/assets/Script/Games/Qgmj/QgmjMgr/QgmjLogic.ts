...
//与服务器段已知的牌字典
export default class QgmjLogic extends BaseMgr
{  
    static dicetime=16; 
    opcardarr=null;
    handcards=null;
    ...
    { 
        ...
        //资源重新加载
        QgmjResMgr.getInstance().clear();  
        var myseatid=RoomMgr.getInstance().getMySeatId(); 
        this.myself.setEvents(msg.events);
        this.myself.replaceJin();
        QgmjResMgr.getInstance().setJin(this.jin)
    }
    tuoGuan( bvalue)
    {
        // body
    ...
    resetData(  )
    {
        ...
        this.process=QgmjDef.process_ready;
    }
    //玩家操作
    onOp(msg)
    {
        ...
        else if(op==QgmjDef.op_qianggang_hu){
            this.op_qianggang_hu(msg);
        }
        else if( op==QgmjDef.op_peng ){
            this.op_peng(msg);
        }
        else if( op==QgmjDef.op_gang ){
            this.op_gang(msg);
        }
        else if( op==QgmjDef.op_angang ){
            this.op_angang(msg);
        }
        else if( op==QgmjDef.op_chi ){
            this.op_chi(msg);
        }
        else if( op==QgmjDef.op_zimo ){
            this.op_zimo(msg); 
        }
    ...
    {
        this.process=msg.process; 
        if (this.process==QgmjDef.process_dingzhuang){
            this.process_dingzhuang(msg);
        }
        else if( this.process==QgmjDef.process_fapai){
            this.process_fapai(msg);
        }
        else if( this.process==QgmjDef.process_buhua){
            this.process_buhua(msg);
        }
        else if( this.process==QgmjDef.process_kaijin){
            this.process_kaijin(msg); 
        }
        else if( this.process==QgmjDef.process_ready){
    ...
    insertWater (score) {
        ...
        this.notify_msg('room.roomHandler.qgmjcallscore', data);
    }
}
