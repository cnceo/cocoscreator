...
var rescfg={}; 
rescfg[QgmjDef.event_hu]='mj_room_option_hu';//胡
rescfg[QgmjDef.event_qianggang_hu]='mj_room_option_qgh';//抢杠胡
rescfg[QgmjDef.event_angang]='mj_room_option_gan';//暗杠
rescfg[QgmjDef.event_bugang]='mj_room_option_gan';//补杠
rescfg[QgmjDef.event_gang]='mj_room_option_gan';//杠
rescfg[QgmjDef.event_peng]='mj_room_option_pen';//碰
rescfg[QgmjDef.event_chi]='mj_room_option_chi';//吃
rescfg[QgmjDef.event_zimo]='mj_room_option_zm';//自摸 
rescfg[QgmjDef.event_sanjindao]='mj_room_option_sjd';//三金倒
rescfg[QgmjDef.event_danyou]='mj_room_option_yj';//游金
rescfg[QgmjDef.event_shuangyou]='mj_room_option_sy';//双游
rescfg[QgmjDef.event_sanyou]='mj_room_option_sany';//三游
rescfg[QgmjDef.event_bazhanghua]='mj_room_option_bzh';//八张花

var animcfg = {};
animcfg[QgmjDef.event_hu]='Btnhu';//胡
animcfg[QgmjDef.event_qianggang_hu]='Btnqiangganghu';//抢杠胡
animcfg[QgmjDef.event_angang]='Btngang';//暗杠
animcfg[QgmjDef.event_bugang]='Btngang';//补杠
animcfg[QgmjDef.event_gang]='Btngang';//杠
animcfg[QgmjDef.event_peng]='Btnpeng';//碰
animcfg[QgmjDef.event_chi]='Btnchi';//吃
animcfg[QgmjDef.event_zimo]='Btnzimo';//自摸 
animcfg[QgmjDef.event_sanjindao]='Btnsanjindao';//三金倒
animcfg[QgmjDef.event_danyou]='Btnyoujin';//游金
animcfg[QgmjDef.event_shuangyou]='Btnshuangyou';//双游
animcfg[QgmjDef.event_sanyou]='Btnsanyou';//三游
animcfg[QgmjDef.event_bazhanghua]='Btnbazhanghua';//八张花

//MVC模块,
const {ccclass, property} = cc._decorator;
let ctrl : QgmjMjEventCtrl;
//模型，数据处理
class Model extends BaseModel{
	...
	updateMySelf(){ 
		...
		this.myself=QgmjLogic.getInstance().players[seatid];
	}
	setCurEventIndex(index)
	{
		this.curEvent=this.myself.events[index];
		var cur_op=QgmjDef.op_cfg[this.curEvent] 
		this.carddatas=null;
		if (cur_op==QgmjDef.op_chi){
			this.carddatas=this.myself.getCardsCandChi(); 
		}
		else if (cur_op==QgmjDef.op_angang){
			this.carddatas=this.myself.getCardsCanAnGang();  
		} 
		else if (cur_op==QgmjDef.op_bugang){
...
class View extends BaseView{
	...
	recover(){
		...
		if(this.model.myself.state!=QgmjDef.state_event){
			return;
		}
		// body 
	...
	show(  ){
		...
		for(let i = 0;i<this.ui.btnOpArr.length;++i)
		{
			...
			if(i<eventLength)
			{
				...
				//console.log("name=",`resources/Games/Qgmj/op/${name}.png`)
				//var texture=cc.loader.getRes(cc.url.raw(`resources/Games/Qgmj/op/${name}.png`))  
	...
	showSubSel()
	{
		...
		var cur_op=QgmjDef.op_cfg[event]
		if (cur_op==QgmjDef.op_chi ){ 
			this.updateChi();
		} 
		else if (cur_op==QgmjDef.op_angang ){ 
	...
	updateHu(  ){
		...
		for (let i=0;i<cardpairs.length;++i){
			...
			for (let k = 0;k<cardarr.length;++k){ 
                ...
                let texture = QgmjResMgr.getInstance().getCardTextureByValue(cardvalue);
                let frame = new cc.SpriteFrame(texture);
	...
	updatePeng(  ){
		...
		var cardvalue=QgmjLogic.getInstance().curcard;
		for (var j =0;j<3;++j){  
			var face=cardface[j];   
			let texture = QgmjResMgr.getInstance().getCardTextureByValue(cardvalue);
			let frame = new cc.SpriteFrame(texture);
...
//c, 控制
@ccclass
export default class QgmjMjEventCtrl extends BaseCtrl {
	//这边去声明ui组件
	...
	btn_cancel_cb(  ){
		// body
		QgmjLogic.getInstance().playerCancel()
		this.view.clear()
	}
	//end
	playerOp(id){
		// body
		if (this.model.curEvent==QgmjDef.event_chi){  
			var chiinfo=this.model.carddatas[id];
			var index=chiinfo.index; 
			QgmjLogic.getInstance().playerOp(this.model.curEvent,index);
		}
		else if ((this.model.curEvent==QgmjDef.event_angang) || (this.model.curEvent==QgmjDef.event_bugang)){ 
			var card=this.model.carddatas[id];
			QgmjLogic.getInstance().playerOp(this.model.curEvent,card); 
		} 
		else {
			QgmjLogic.getInstance().playerOp(this.model.curEvent);
		}
		this.view.clear()
	}
	touchPanel(index){ 
