...
class View extends BaseView{
	...
	updateBuGang(card)
	{
		for (let index = 0;index<this.model.player.opcards.length;++index){  
			...
			if(opcard.op==QgmjDef.op_bugang && opcard.value==card)
			{  
	...
	addDoorCard(index){
		...
		else if (opcard.op==QgmjDef.op_peng){ 
			var value=opcard.value
			for (var i = 0;i<3;++i){
				var card=panel.getChildByName(`MaJiang_${i}`);
				card.active=true;
		...
		else if (opcard.op==QgmjDef.op_gang){ 
			var value=opcard.value
			for (var i = 0;i<4;i++){
				var card=panel.getChildByName(`MaJiang_${i}`);
				card.active=true;
		...
		else if (opcard.op==QgmjDef.op_bugang){ 
			var value=opcard.value
			for (var i = 0;i<4;i++){
				var card=panel.getChildByName(`MaJiang_${i}`);
				card.active=true;
		...
		else if (opcard.op==QgmjDef.op_angang){ 
			var value=opcard.value
			for (var i = 0;i<4;++i){
				var card=panel.getChildByName(`MaJiang_${i}`);
				card.active=true;
				...
				sign.active = flag;
				let texture = QgmjResMgr.getInstance().get3DCardTextureByValue(flag ? value : 666);
				let frame = new cc.SpriteFrame(texture);
	...
	recoverPeng(card)
	{
		for (let index = 0;index<this.model.player.opcards.length;++index){  
			let opcard=this.model.player.opcards[index];
			if(opcard.op==QgmjDef.op_peng && opcard.value==card)
			{  
				var panel=this.doorCardPanel[index] 
				var value=opcard.value
				var card=panel.getChildByName(`MaJiang_3`);
	...
	updateCards(msg){
		// body
		if(msg.event==QgmjDef.event_bugang)
		{
			this.updateBuGang(msg.card);
		}
		else if(msg.event==QgmjDef.event_qianggang_hu)
		{
...
//c, 控制
@ccclass
export default class QgmjDoorCardCtrl extends BaseCtrl {
	//这边去声明ui组件 
	@property
    seatId:Number = 0;
	//声明ui组件end
	...
	onOp(msg){
		...
		//吃碰杠等列表
		let leagleOps=[QgmjDef.op_chi,QgmjDef.op_peng,QgmjDef.op_gang,QgmjDef.op_angang,QgmjDef.op_bugang]
