	...
	Show () : void {
		...
		if (data.hu) {
			...
			if (hus.jincount) {
				...
				this.ui.node_GoldCardImg.spriteFrame = new cc.SpriteFrame(QgmjResMgr.getInstance().getCardTextureByValue(hus.jincount));
			}
		}
		//牌阵
		...
		for (let j = 0; j < cards.length; j ++) {
			...
			let id = cards[j];
			let textrue = QgmjResMgr.getInstance().getCardTextureByValue(id);
