...
import BullConst from "../../Tbnn/BullMgr/BullConst";
import QznnConst from "./QznnConst";
let NiuNiu = require("../../Tbnn/BullMgr/NiuNiuFP.js");
//牛牛卡牌
...
export default class QznnCardsMgr extends BaseMgr{
    ...
    resetCardsToTarget (cardsList:Array<cc.Node>, targetPos:cc.Vec2){
        let giveNum = cardsList.length,
            ...
            cardNode,
            endPos,
            offRate = QznnConst.config.minCardOffWRate;
        for(i = 0; i < giveNum; i ++){
    ...
    //单例
    private static _instance:QznnCardsMgr;
    public static getInstance ():QznnCardsMgr{
        if(!this._instance){
            this._instance = new QznnCardsMgr();
        }
